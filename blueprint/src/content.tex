...
\begin{definition}
  ...
  \begin{enumerate}
    \item for every nonzero $x$ in $F$, exactly one of $x$ or $-x$ is a square in $F$, and
    \item every odd-degree polynomial over $F$ has a root in $F$.
  \end{enumerate}
\end{definition}

\begin{lemma}
  \label{lem:RCF_of_ord}
  \uses{def:RCF}
...
\end{lemma}
\begin{proof}
  Exactly one of $1$ and $-1$ is a square, so $-1$ is not a square.
\end{proof}
...
\end{lemma}
\begin{proof}
  \uses{lem:unique_ord_cond, lem:RCF_real}
  This is Lemma \ref{lem:unique_ord_cond}.
\end{proof}
...
\end{lemma}
\begin{proof}
  \uses{lem:RCF_real,thm:ord_iff_real}
  \leanok
...
\begin{proof}
  \leanok
  Let $a$ in $R$, and suppose $a$ is not a square. Then $R(\sqrt{a})\cong R(i)$. Suppose $i$ maps to $x+y\sqrt{a}$ for some $x,y\in R$; then $-1=x^2+ay^2+2xy\sqrt{a}$. Comparing coefficients, we get $-1=x^2+ay^2$. Since $-1$ is not a square, $y$ must be nonzero, and so $-a=(x/y)^2$ is a square.
...
\begin{proof}
  \uses{lem:RCF_of_ord}
  We use Lemma \ref{lem:RCF_of_ord}. Let $a\in R$ be non-negative, and consider the polynomial $f=X^2-a$. Then $f(0)=-a\leq0$, but $f(a+1)=a^2+a+1>0$. By the intermediate value property, $f$ has a root in $R$, and so $a$ is a square in $R$.
...
\end{theorem}
\begin{proof}
  \uses{lem:RCF_of_ord,cor:ext_ord_to_adj_sqrt,lem:ext_ord_odd_deg}
  We use Lemma \ref{lem:RCF_of_ord}. 
...
\begin{proof}
  ...
  Since ordered fields are real, we are done by Theorem \ref{thm:ord_max_imp_RCF}.
\end{proof}
...
\begin{theorem}
  \label{thm:RCF_tfae}
  \uses{def:RCF}
  Let $R$ be a real field. TFAE:
  \begin{enumerate}
    \item $R$ is real closed.
    \item $\bar{R}=R(i)$ (and $-1$ is not a square in $R$).
    \item $R$ has no nontrivial real algebraic extensions.
  \end{enumerate}
\end{theorem}
\begin{proof}
  \uses{cor:RCF_ac,lem:FTAlg_converse,lem:RCF_max,cor:real_max_imp_RCF}
\end{proof}
...
We can go much further than Lemma \ref{lem:FTAlg_converse}. The following is a weak form of the Artin-Schreier theorem. Removing the condition on characteristic is possible, but requires some more involved algebra.
