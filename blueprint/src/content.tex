...
% TODO : either generalise the section on ring orderings, or remove it and just state the existence conditions
...
\begin{definition}
  ...
  \lean{IsSemireal}
  \leanok
  A field is real if $-1$ is not a sum of squares.
\end{definition}
...
\begin{lemma}
  ...
  \uses{def:semireal}
  \lean{IsSemireal.existsUnique_isStrictOrderedRing}
  \leanok
...
\begin{proof}
  ...
  is a sum of squares. We are done by Corollary \ref{cor:unique_ord_cond_ordered}.
\end{proof}

There is a corresponding algebraic characterisation of an ordered field extension. Note that, since real fields have characteristic 0, their algebraic extensions are separable.
...
\begin{lemma}
  ...
  \uses{def:RCF}
  \lean{IsRealClosed.unique_isStrictOrderedRing}
  \leanok
...
\begin{proof}
  \leanok
  \uses{cor:unique_ord_cond_ordered}
  Follows directly from Corollary \ref{cor:unique_ord_cond_ordered}.
\end{proof}
...
\begin{lemma}
  ...
  \uses{def:RCF}
  \lean{IsRealClosed.even_finrank_extension}
  \leanok
  Nontrivial finite extensions of $R$ have even degree.
\end{lemma}
\begin{proof}
  \leanok
...
\begin{lemma}
  \label{lem:ext_deg_2}
  \uses{def:RCF}
  \lean{IsRealClosed.isAdjoinRootIOfFinrankExtensionEqTwo}
  The field $R(i)$ is the unique quadratic extension of $R$.
\end{lemma}
\begin{proof}
  Let $K/R$ be a quadratic extension. Since $\text{char}(R)\neq2$, we have $K\cong R(\sqrt{a})$ for some $a\in R$. Since nonnegative elements of $R$ are squares, $a$ is negative and so $-a$ is a square in $R$. Rescaling by $\sqrt{-a}^{-1}$, we get $R(\sqrt{a})\cong R(i)$.
\end{proof}

\begin{lemma}
  \label{lem:Ri_ext_deg_2}
  \uses{def:RCF}
  \lean{IsRealClosed.finrank_adjoinRoot_i_extension_neq_two}
  There is no quadratic extension of $R(i)$.
\end{lemma}
\begin{proof}
  Since $\text{char}R(i)\neq 2$, it suffices to show that every element of $R(i)$ is a square. Indeed, take $x=a+bi\in R(i)$ with $a,b\in R$. If $b=0$, then either $a\geq0$ and so $x$ is a square in $R$, or $a\leq0$ and so $a=(i\sqrt{-a})^2$ is a square in $R$. Now let $b\neq0$. Then we compute $x=(c+di)^2$, where
...
\end{theorem}
\begin{proof}
  \uses{lem:ext_deg_2, lem:Ri_ext_deg_2, lem:ext_ord_odd_deg}
...
\begin{corollary}
  \label{cor:FTAlg_alg}
  \uses{def:RCF}
  \lean{IsRealClosed.algebraic_extension_classify}
...
\begin{corollary}
  \label{cor:RCF_ac}
  \uses{def:RCF}
  \lean{IsRealClosed.isAdjoinRootIOfIsAlgClosure}
  $\bar{R}=R(i)$.
\end{corollary}
\begin{proof}
  \uses{cor:FTAlg_alg}
...
\begin{lemma}
  ...
  \uses{def:RCF}
  \lean{IsRealClosed.of_isAdjoinRoot_i_or_1_extension}
  \leanok
  Suppose $R$ is an ordered field whose only nontrivial finite extension is $R(i)$. Then $R$ is real closed.
\end{lemma}
\begin{proof}
  \leanok
...
\begin{proof}
  \uses{cor:FTAlg_alg}
  Since $-1$ is a square in $R(i)$, the field $R(i)$ is not formally real and therefore cannot be ordered. We are done by Theorem \ref{cor:FTAlg_alg}.
\end{proof}
...
\begin{theorem}
  ...
  \uses{def:RCF}
  \lean{IsRealClosed.of_maximalIsOrderedAlgebra}
  \leanok
  Let $R$ be an ordered field maximal with respect to ordered algebraic extensions. Then $R$ is real closed.
\end{theorem}
\begin{proof}
  \leanok
  \uses{cor:ext_ord_to_adj_sqrt,lem:ext_ord_odd_deg}
  Let $a\in R$ be non-negative, and suppose $a$ is not a square. By Corollary \ref{cor:ext_ord_to_adj_sqrt}, there is an ordering making the nontrivial extension $R(\sqrt{a})/R$ ordered, contradicting maximality.$_\#$ Therefore $a$ is a square in $R$.
...
\begin{theorem}
  ...
  \begin{enumerate}
    ...
    \item $R$ is maximal with respect to algebraic extensions by ordered fields.
  \end{enumerate}
\end{theorem}
\begin{proof}
  \uses{cor:RCF_ac,lem:FTAlg_converse,lem:IVP_poly,thm:IVP_poly_imp_RCF,lem:RCF_max,thm:ord_max_imp_RCF}
\end{proof}
...
\end{lemma}
\begin{proof}
  \uses{thm:ord_iff_real,lem:FTAlg_converse,lem:RCF_ord_unique,lem:sumsq_is_sq}
  We are very nearly done by Theorem \ref{thm:ord_iff_real}, Lemma \ref{lem:FTAlg_converse}, and Corollary \ref{lem:RCF_ord_unique}; it remains to show that $R$ is real.

  We have $\bar{R}\cong R(i)$. Since $R(i)/R$ is nontrivial, $-1$ is not a square in $R$. Since every element of $R(i)$ is a square, Lemma \ref{lem:sumsq_is_sq} says that $-1$ is not a sum of squares in $R$.
\end{proof}
