...
\begin{proof}
  \uses{thm:ext_ord_cond}
  \leanok
  Since $\operatorname{char} R=0$, we can apply the primitive element theorem. Let $K=F(\alpha)$ for some $\alpha\in K$, and let $f$ be the minimal polynomial of $\alpha$ over $K$. Then $\deg f=[K:F]$ is odd. By Theorem \ref{thm:ext_ord_cond}, we need to show that the congruence
