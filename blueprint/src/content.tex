...
\end{lemma}
\begin{proof}
  \uses{lem:unique_ord_cond,lem:sum_sq_is_sq}
  Follows directly from Lemmas \ref{lem:unique_ord_cond} and \ref{lem:sum_sq_is_sq}.
\end{proof}
...
\begin{proof}
  ...
  Note that $\sqrt{a^2+b^2}$ is in $R$ by Lemma \ref{lem:sum_sq_is_sq}, and $c\neq0$ because otherwise $-a=\sqrt{a^2+b^2}$ and so $b=0$.$_\#$
\end{proof}
...
\begin{proof}
  ...
  By Lemma \ref{lem:FTAlg_converse}, it suffices to show that $\bar{R}\cong R(i)$.
