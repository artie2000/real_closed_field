...
\begin{lemma}
  ...
  \uses{def:ordering, lem:max_preord_is_ord}
  \leanok
  \lean{RingPreordering.exists_le_isPrimeOrdering}
  Let $P$ be a preordering on a field $F$. Then $P$ is contained in an ordering.
\end{lemma}
\begin{proof}
  Apply Zorn's lemma to the preorderings of $F$ containing $P$; by Lemma \ref{lem:max_preord_is_ord}, this yields an ordering containing $P$.
\end{proof}
...
\begin{lemma}
  \label{lem:ord_max}
  \uses{def:ordering, lem:linord_fieldord_bij}
  \leanok
...
\begin{lemma}
  ...
  \uses{def:semireal, def:ordered_field}
  Let $F$ be a real field, and let $a\in F$ be non-negative. Then $F(\sqrt{a})$ is real.
\end{lemma}
\begin{proof}
...
\begin{lemma}
  \label{lem:ext_ord_to_adj_sqrt}
  Let $F$ be a ordered field, and let $a\in F$ be non-negative. Then there is an ordering on $F(\sqrt{a})$ extending the ordering on $F$.
\end{lemma}
\begin{proof}
  TODO
\end{proof}
...
Lemma \ref{lem:ext_ord_to_adj_sqrt} allows us to construct ordered algebraic extensions. There is an easier way to construct ordered extensions if we don't care about them being algebraic.
...
\end{corollary}
\begin{proof}
  Let $a\in R$ be non-negative. By Lemmas \ref{lem:adj_sqrt_real} and \ref{lem:ext_ord_to_adj_sqrt}, there is an ordering on $R(\sqrt{a})$ making $R(\sqrt{a})/R$ into an ordered extension. By maximality, $R(\sqrt{a})\cong R$, and so $a$ is a square in $R$. By Corollary \ref{cor:unique_ord_cond_ordered}, $R$ is ordered uniquely.
