...
\begin{lemma}
  \label{lem:RCF_max}
  \uses{def:RCF}
  \lean{IsRealClosed.maximal_isOrderedField}
  \leanok
  $R$ is maximal with respect to algebraic extensions by ordered fields.
\end{lemma}
\begin{proof}
  \uses{cor:FTAlg_alg, thm:ord_iff_real}
  Since $-1$ is a square in $R(i)$, the field $R(i)$ is not formally real and therefore cannot be ordered (by Theorem \ref{thm:ord_iff_real}). We are done by Corollary \ref{cor:FTAlg_alg}.
\end{proof}
