...
Note that the proofs in this section don't correspond perfectly to the Lean as I have generalised the work to arbitrary commutative rings.
...
\end{lemma}
\begin{proof}
  Go away and check the axioms are equivalent.
\end{proof}
...
\begin{lemma}
  \label{lem:ord_iff_max}
  \uses{def:ordering}
  \leanok
  \lean{RingPreordering.maximal_iff_isOrdering}
  Let $P$ be a preordering on a field $F$. Then $P$ is maximal if and only if it is an ordering.
\end{lemma}
\begin{proof}
  Suppose $P$ is not an ordering. Then there is some element $a\in F$ such that $a,-a\notin P$. One can check that the semiring generated by $P$ and $a$ does not contain -1, and is therefore a strictly bigger preordering.

  Conversely, suppose $P$ is not maximal. Then there is some preordering $Q\supsetneq P$ on $F$. Find $x\in Q\setminus P$. If $P$ is an ordering, then $-x\in P$, and so $-1=-x/x\in Q$.$_#$
\end{proof}

\begin{lemma}
  \label{lem:preord_ext}
  \uses{def:ordering, lem:ord_iff_max}
  \leanok
  \lean{RingPreordering.exists_le_isPrimeOrdering}
  Let $P$ be a preordering on a field $F$. Then $P$ is contained in an ordering.
\end{lemma}
\begin{proof}
  Apply Zorn's lemma to the preorderings of $F$ containing $P$; by Lemma \ref{lem:ord_iff_max}, this yields an ordering containing $P$.
\end{proof}

\begin{theorem}
  \label{thm:ord_iff_real}
  \uses{def:ordered_field, def:semireal, lem:linord_fieldord_bij, lem:preord_ext}
  \leanok
  \lean{Field.nonempty_isOrderedRing_iff_isSemireal}
  A field can be ordered if and only if it is real.
\end{theorem}
\begin{proof}
  Let $F$ be a field. If $F$ has an ordering $O$, then $O$ contains all sums of squares in $F$, but not $-1$, so $-1$ cannot be a sum of squares. Conversely, if $-1$ is not a sum of squares in $F$, then the sums of squares in $F$ form a preordering; by Lemma \ref{lem:preord_ext}, there is an ordering on $F$.
\end{proof}

\begin{lemma}
  \label{lem:unique_ord_cond}
  \uses{def:semireal, def:ordering, }
...
\end{lemma}
\begin{proof}
  Observe that every preordering on $F$ must contain all sums of squares in $F$. (TODO: reorganise to be closer to the Lean) Since $-1$ is not a sum of squares in $F$, the sums of squares themselves form a preordering. By assumption, this preordering is in fact an ordering; by Lemma \ref{lem:ord_iff_max}, orderings are maximal as preorderings.
\end{proof}
